...
class MeasureTypeValidator < TradeTariffBackend::Validator
  ...
  validation :MT3, 'When a measure type is used in a measure then the validity period of the measure type must span the validity period of the measure.', on: [:create, :update] do |record|
  ...
  validation :MT7, 'A measure type can not be deleted if it is used in a measure.', on: [:destroy] do |record|
    record.measures.none?
  end

  validation :MT10, 'The validity period of the measure type series must span the validity period of the measure type.' do |record|
