    ...
    private
      ...
      def check_section_and_chapter!
        unless chapter_exists_for_nomenclature?(settings.goods_nomenclature_item_id)
          @errors[:goods_nomenclature_item_id] = errors_translator(:chapter_doenst_exist)
          @errors_summary = errors_translator(:chapter_doenst_exist)
        end
      end
      ...
      def chapter_exists_for_nomenclature?(goods_nomenclature_item_id)
        ...
        section_chapter = GoodsNomenclature.actual(include_future: true).where(goods_nomenclature_item_id: goods_nomenclature_item_id[0..3].ljust(10, '0'), status: 'published').first
